...
class FabricService(
    ...
    private val storage : LoomRepository
) {
  ...
  /**
   * Sets a model as 'active' (true) which means it can be used as a model when requesting a new Fabric.
   *
   * @param name fabric model name to modify.
   * @return the modified fabric model.
   * @throws LoomException if the named fabric model does not exist.
   */
  fun activateFabricModel(name: String): FabricModel =
      (getFabricModel(name) ?: throw LoomException(404)).apply {
        val updated = copy(active = true)
        storage.updateFabricModel(updated)
      }

  /**
   * Sets a model as 'inactive' (false) which means it cannot be used as a model when requesting a new Fabric.
   *
   * @param name fabric model name to modify.
   * @return the modified fabric model.
   * @throws LoomException if the named fabric model does not exist.
   */
  fun deactivateFabricModel(name: String): FabricModel =
      (getFabricModel(name) ?: throw LoomException(404)).apply {
        val updated = copy(active = false)
        storage.updateFabricModel(updated)
      }

  /**
   * Queries whether a [FabricModel] is active or inactive. If a [FabricModel] is not found with the given name then it
   * is assumed to be inactive from the perspective of this API call.
   *
   * @param name fabric model name to modify.
   * @return whether a [FabricModel] is active (active) or inactive (false or null).
   */
  fun isFabricModelActive(name: String) = getFabricModel(name)?.active ?: false

  fun getFabricSpec(name: String): FabricSpec? = storage.getFabricSpecByName(name)

  fun getClusterKubeconfig(name: String): String? {
    val spec = getFabricSpec(name) ?: throw LoomException(404)

    val workspace = getWorkspace(name)
    val kops = Kops2.newKops(workspace, amazon.stateStorageBucketName)
    return kops.exportClusterContext(spec.clusterDomain)
  }

  fun getFabricModel(name: String): FabricModel? = storage.getFabricModelByName(name)

  fun listFabricModels() = storage.listFabricModels()

  fun listFabricSpecs() = storage.listFabricSpecs()

  fun createFabricModel(model: FabricModel) =
      synchronized(this) {
        if (!storage.fabricModelExists(model.name)) {
          storage.createFabricModel(model)
        } else {
  ...
  fun removeResourceFromFabric(fabricName: String, resourceName: String): FabricSpec {
    return synchronized(this) {
      storage.getFabricSpecByName(fabricName)
          ?.let { current ->
            ...
            if (current != updated) {
              ...
              storage.updateFabricSpec(updated)
              updated
            } else {
              current
            }
  ...
  fun addResourceToFabric(fabricName: String, config: ResourceConfig): FabricSpec {
    return synchronized(this) {
      val fabricSpec = storage.getFabricSpecByName(fabricName) ?: throw LoomException(404)

      storage.getBackingResourceModelByName(config.model)
          ?.let {
            ...
            storage.updateFabricSpec(updatedFabric)
            updatedFabric
          }
          ?: throw LoomException(404)
    }
  ...
  fun createFabric(config: FabricConfig): FabricSpec {
    return synchronized(this) {
      ...
      val spec = storage.createFabricSpec(
          assembleFabricSpec(
              model  = getFabricModel(normalizedConfig.model) ?: throw LoomException(404),
              config = config
          ))
